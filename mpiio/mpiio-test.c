     ...
     char *argv[];
{
  ...
  if (i_am_the_master) 
	...
	switch(c) 
	  ...
	  case 'r': 
	    ...
	    printf("Each process will write %d blocks of byte and repeat %d\ntotal:%d\n", 
  ...
  MPI_Bcast(&number_of_blocks,   1, MPI_LONG_LONG_INT, MASTER_RANK, MPI_COMM_WORLD);
  MPI_Bcast(&block_size_in_byte, 1, MPI_LONG_LONG_INT, MASTER_RANK, MPI_COMM_WORLD);
  MPI_Bcast(&number_of_repeat,   1, MPI_LONG_LONG_INT, MASTER_RANK, MPI_COMM_WORLD);
  MPI_Bcast(&filename_length,    1, MPI_LONG_LONG_INT, MASTER_RANK, MPI_COMM_WORLD);
...
if (i_am_the_master) 
 ...
 while (j<number_of_repeat)
   {
     my_offset = (my_rank+j) * block_size_in_byte;     
...
#ifdef DEBUG
     printf("%3d: wrote %d integers number_of_repeat:%d/%d\n", my_rank, count,j,number_of_repeat);
#endif
     MPI_File_get_position(fh, &my_current_offset);
#ifdef DEBUG
     printf ("%3d: my current offset is %lld\n", my_rank, my_current_offset);
